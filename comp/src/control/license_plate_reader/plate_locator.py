...
class Plate_Locator(object):
    ...
    def __init__(self):
        ...
        self.real_plate_h = 225          # actual: 45
    ...
    def locate_plate(self, gr_cap, count_loop_save):   
        ...
        # print(confidences)
        if len(rects) > 0:
            # start_nonmax_time = time.time()
            boxes = non_max_suppression(np.array(rects), probs=confidences)
            # print("this is non max time")
            # print(time.time() - start_nonmax_time)
        ...
        if count_loop_save > 0:
            ...
            if (parking_indices_bottomL_y - parking_indices_topL_y > 10) and (parking_indices_bottomR_y - parking_indices_topR_y > 10):
                ...
                # print(indices_topL_y, indices_topR_y, indices_bottomR_y, indices_bottomL_y)
                    ...
                    four_points_trans_reshaped = np.float32([[0,0],[self.real_plate_w - 1,0],[self.real_plate_w - 1, self.real_plate_h - 1],[0,self.real_plate_h - 1]]).reshape(-1,1,2)
                    ...
                    parking_image = cv2.warpPerspective(merged_cropped_layer_top, M_parking, (self.real_plate_w, self.real_plate_h))
                    ...
                    plate_image = cv2.warpPerspective(merged_cropped_layer_bottom, M_plate, (self.real_plate_w, self.real_plate_h))

                    # print("this is time taken right before saving")
                    # print(time.time() - start_processing_time)
            ...
            # print("this is procesising time")
            # print(time.time() - start_processing_time)
